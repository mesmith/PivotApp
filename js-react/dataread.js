...
const dataread = function() {
  ...
  //
  const readDataset = function(handler, dataset, filter, loadTable, datapointCol){
    ...
    //
    if( isCSV(dataset) ){
      csvReadDataset(dataset, handler);
    } else {
      mongoReadDataset(dataset, datapointCol, filter).done((res) => {
        if (res.categoricalValues && res.drawingData) {
          const cookedData = process(res.drawingData, loadTable);
          handler(dataset, res.categoricalValues, res.drawingData, cookedData);
        } else {
          handler(dataset, {}, [], []);
        }
      });
  ...
  //
  const csvReadDataset = function(dataset, handler){
    return readCSVData([], dataset, function(rawData){
      const columns = metadata.getColumnsByAttrValue('type', 'Categorical')
      const categoricalValues = utils.getAllUniqueValues(withAverages, columns);
      const cookedData = process(rawData, null);

      handler(dataset, categoricalValues, cookedData);
    });
  }
  ...
  //
  // The function reads the CSV dataset, and calls onCSV with the
  // data when it's done.
  //
  const readCSVData = function(datasets, dataset, onCSV){

    // If the dataset has changed from last time, load the new csv file
    //
    if( dataset!=datasets.slice(-1) ){
      const url = "data/" + dataset;
      d3.csv(url, onCSV);
  ...
  const getDatasetTransform = function(data) {
    ...
    const numericFields = metadata.getNumerics();
  ...
  const withDates = function(data){
    ...
    const outputCols = dateStringCols.reduce((i, j) => {
      return {...i, ...{[j] : metadata.getAttrValue(j, 'output')}};
    }, {});
    return data.map(i => {
  ...
  const process = function(data, loadTable) {
    const res = withCalculatedFields( // must be last, after synethetic dataset!
      getDatasetTransform(
      withAverages(
      withDates(
