...
const tooltip = function(){
  ...
  function doTooltipAlias(bConvertAlias, bIgnore, tooltipPivots, datapointCol,
    ...
    const titleFactory = function(datapointCol){
      return function(){
        ...
        return header.join('') + '<div>' + body.join('') + '</div>' ;
      }
    }
    const options = {
      gravity: 'n',
  ...
  function getHeader(thisData, alias, bConvertAlias, datapointCol, bIgnore){
    if( typeof thisData[alias]!='undefined' && thisData[alias]!='' ){
      if( bConvertAlias ){
        return ['<div class="bold center">' +
            datapoint.formatDatapointRep(datapointCol, thisData[alias]) + 
            '</div>'];
      } else {
        return ['<div class="bold center">' + thisData[alias] + '</div>'];
      }
    } else {
      if( !bIgnore ){
        return ['<h3>Unknown ' + alias + '</h3>',
          '<div class="bold center" style="padding: 3px; float: none">Unknown ' + 
            alias + 
          '</div>'];
      }
    }
