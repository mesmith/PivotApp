...
const transforms = function(){
  ...
  function mapper(data, where, datapointCol, animationCol){
    ...
    return applyWhereClause(data, where, datapointCol).map(function(datum){
      ...
      //
      const newrec = getCategoricalsAndStrings(datum, pivoted, strings);
  ...
  // This converts 'datum' into a record that has a value for
  // each pivot, as well as for each string in 'strings'.
  //
  const getCategoricalsAndStrings = function(datum, pivotedList, strings){

    // Generate the pivoted values.
    // Apply aliases now.
    //
    // Note how we avoid the object cloning here, which is
    // much slower than using _.object() on an array of name/value arrays.
    //
    return _.object(pivotedList.concat(
      strings.map(function(x){return [metadata.getAlias(x), datum[x]];})
    ));
  }
  ...
  function reducer(mapped){
    ...
    const aggregate_cols = Object.keys(outer.$p);

    return Object.keys(data).map(function(x){
      return {$k: x, $v: data[x]};
    }).filter(function(x){
      return _.isArray(x.$v);
    }).map(function(x){

      // x.$v is an array of the form
      //   [ {COL1: NUM1, COL2: NUM2, ...}, ... ]
      // for a particular (presumably categorical) variable.
      // The task is to calculate SUM(NUMi) for each COLj.
      //
      // This must perform very quickly.
      //
      const newval = getAggregateValues(x.$v, aggregate_cols);

      // Un-serialize the compound key, adding the object's name-value 
      // pairs to newval
      //
      return {...newval, ...JSON.parse(x.$k)};
    });
  }
  ...
  function reducerForce(mapped, bStatus, datapointCol, d3geom){
    ...
    const children = Object.keys(data).map(function(key){
      ...
      // Aggregate the enumeration (pivot) data
      //
      const aggObj = getAggregateValues(data[key], aggregate_cols);
  ...
  // where each value is the array of categorical values for the column name.
  ...
  const outerReducer = function(input){
    return input.reduce(function(v1, v2, idx, array){
      ...
      //
      const newValue = (v2.key==v1.$k) ?  v1.$v.concat([v2.value]) : [v2.value];
      const last = (idx==array.length-1);
      const all = ( v2.key!=v1.$k && v1.$k )?  {...v1.$all, ...{[v1.$k]: v1.$v}} : v1.$all;
      const all2 = last ?  {...all, ...{[v2.key]: newValue}} : all;

      return {
        $k: v2.key,
        $v: newValue,
        $all: all2,
        $p: addPivotObject(v1.$p, v2),
      }
    }, {$k: null, $v: [], $all: {}, $p: {}});
  }
