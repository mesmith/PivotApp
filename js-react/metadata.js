...
const metadata = function(){
  ...
  function applyFormat(value, format, type) {
    switch(format) {
      ...
      default:
      {
        return usePrecision(value, type) ? value.toFixed(2) : value;
      }
    }
  }

  function usePrecision(value, type) {
    return type==='Number' && isFloat(value) && !Number.isInteger(value);
  }
  ...
  // Same as above, but filter out columns that are hav 'isAverage' set to true.
  //
  ...
  function getAxisFormat(alias){
    ...
    if (axisFormat === 'million') {
      ...
      return function(d) {return getPercent(d) + '%';}
    } else if (axisFormat === 'date') {
      return function(d) {
        return utils.getMonthFromDateString(d);
      }
    }
    try {
      switch( col.type ){
  ...
  function getCategoricalList(categoricalValues, filterAttr) {
    ...
    return [].concat.apply([],
      Object.keys(categoricalValues).filter(i => {
